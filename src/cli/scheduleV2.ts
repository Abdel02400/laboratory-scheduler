...
import { planifyLab } from '@/core/v2/planifyLab';
...
function main(): void {
    ...
    if (!values.file) {
        throw new Error('Usage: pnpm schedule:v2 --file=<path-to-json> [--output=<path-to-write>]');
    }
    ...
    if (values.output) {
        ...
        console.info(`Wrote v2 output to ${values.output}`);
    } else {
